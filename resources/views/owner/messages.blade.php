  ...
  <div class="min-h-screen flex items-start justify-center py-10 px-4">
    <div class="w-full max-w-6xl">
      ...
      <header class="mb-8 text-center">
        <h1 class="text-4xl font-bold header-glow">💬 Owner Conversations</h1>
        <p class="text-sm text-green-200 mt-2">Manage chats with your renters in a glassy, green interface</p>
      </header>
<!-- Back Button -->
    <button onclick="history.back()"
      class="absolute top-6 left-6 w-10 h-10 rounded-full flex items-center justify-center back-btn shadow-md hover:scale-105 transition-transform"
      title="Back">
      ...
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <aside class="col-span-1 glass-card rounded-2xl p-5">
          ...
          @php
            $ownerId = auth()->id();
            $renterIds = $messages->where('user_id', '!=', $ownerId)->pluck('user_id')->unique()->values();
          @endphp
          ...
          @else
            <ul class="space-y-3">
              @foreach($renterIds as $renterId)
                @php
                  $repMsg = $messages->firstWhere('user_id', $renterId);
                  $renterName = optional($repMsg->user)->name ?? 'Renter #' . $renterId;
                  $count = $messages->where('user_id', $renterId)->count();
                @endphp
        ...
        <main class="col-span-2 space-y-6">
          @if($renterIds->isEmpty())
            <div class="glass-card rounded-2xl p-6 text-center text-gray-100">
              <p>No renter messages found yet.</p>
            </div>
          @else
            @foreach($renterIds as $renterId)
              @php
                $repMsg = $messages->firstWhere('user_id', $renterId);
                $renterName = optional($repMsg->user)->name ?? 'Renter #' . $renterId;
                $renterPropertyIds = $messages->where('user_id', $renterId)->pluck('property_id')->filter()->unique()->values();
                $conversation = $messages->filter(function($m) use ($renterId, $ownerId, $renterPropertyIds) {
                  if($m->user_id == $renterId) return true;
                  if($m->user_id == $ownerId && $m->property_id && $renterPropertyIds->contains($m->property_id)) return true;
                  return false;
                })->sortBy('created_at')->values();
                $replyPropertyId = $renterPropertyIds->first() ?? null;
              @endphp
              ...
              <section id="conversation-{{ $renterId }}" class="glass-card rounded-2xl p-6">
                ...
                <form class="flex gap-2" method="POST"
                      action="{{ $replyPropertyId ? route('owner.messages.send', $replyPropertyId) : route('owner.messages.send') }}">
                  @csrf
                  <input type="hidden" name="property_id" value="{{ $replyPropertyId }}">
      ...
      <footer class="mt-8 text-center text-xs text-green-200">
        Click a renter from the sidebar to jump to their conversation.
      </footer>
