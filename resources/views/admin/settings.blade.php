...
<div class="max-w-4xl mx-auto bg-white bg-opacity-70 p-8 rounded-2xl shadow-lg backdrop-blur-md">
    <h2 class="text-2xl font-bold text-gray-800 mb-6">Settings</h2>
    <form action="{{ route('profile.update') }}" method="POST">
        @csrf
        <div class="mb-4">
            <label class="text-gray-700 font-medium">Name</label>
            <input type="text" name="name" value="{{ $user->name }}" class="w-full mt-1 px-4 py-2 rounded-lg border focus:ring focus:ring-blue-300">
        </div>
        <div class="mb-4">
            <label class="text-gray-700 font-medium">Email</label>
            <input type="email" name="email" value="{{ $user->email }}" class="w-full mt-1 px-4 py-2 rounded-lg border focus:ring focus:ring-blue-300">
        </div>
